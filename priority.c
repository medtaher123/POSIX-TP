...
void active_wait(double seconds) {
    clock_t start = clock();
    while (1) {
        clock_t now = clock();
...
int main(void)
{
    ...
    SCHED_FIFO, &param); 
    //pthread_self() pointe sur le processus en cours d'exécution, à l'occurrence la fonction main()
    ...
    pthread_attr_setschedparam(&attr, &param);
    pthread_create(&tache2, &attr, fonc, &id2);  
